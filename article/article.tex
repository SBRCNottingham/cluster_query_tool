...
We then use this approach to uncover latent relationships between sets initial query sets.
...
When tested on both real and synthetic datasets our method achieves high levels of classification accuracy in a context specific manner.
\end{abstract}
...
In network science, this methodology is often referred to as \textit{community} detection \cite{fortunato2010community}.
...
This approach can be based on labels that are not first order neighbours in order to find other potentially related vertices.
...
The detected clusters then form the basis of a search algorithm that allows one to compute the relatedness of subsets of nodes.
...
To characterise latent community structure, one of the most popular approaches is to use modularity maximisation \cite{newman2004}.
The modularity quality function of a partition on a graph is given by the equation
...
The Louvain algorithm is selected as it is fast, running in $O(n \log n)$ time complexity \cite{blondel2008fast}, and because it is a greedy algorithm and is guaranteed to stop after finding a locally maximal solution.
...
Consequently, the approach taken here is to use a large but not exhaustive subset of the possible solutions using a suitably large space 2000 solutions for the networks studied in this paper.
\footnote{Initial results indicated that a smaller space of partitions is possible but further work is required to asses the best number of partitions in a practical context}.
...
For this work we would like to capture binary classification  performance, \textit{true positives (tp), true negatives (tn), false positives (fp)} and \textit{false negatives (fn)}, on our datasets of community membership.
...
Consequently, cross validation is either conducted on an exhaustive set of all possible $\binom{|L|}{s}$ unique labellings or 120 seed queries sampled without replacement\footnote{This is equivalent to $\binom{10}{3}$ combinations, an exhaustive sample would not be possible for the larger communities in this study.}, where $L$ is the set of gold standard true labels and $s$ is the size of the randomly selected seed sets.
...
\begin{figure*}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/rocs/eu_email.png}
        \caption{EU email departments}
    \end{subfigure}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/rocs/yeast_ppi.png}
        \caption{Yeast PPI protein complexes}
    \end{subfigure}
    
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/rocs/arabidopsis_ppi_combined.png}
        \caption{Arabidopsis PPI protein complexes}
    \end{subfigure}
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/rocs/ecoli_ppi.png}
...
For all the datasets in this study, binary community classification is high, with an average AUC scores above 0.83, even when using only a single seed node.
...
The method also serves as a useful network reduction tool where the quality of labels is not as well defined.
This visually is demonstrated for the \textit{Arabidopsis PPI} dataset shown in Figure \ref{fig:query_example} with a hand selected gene ontology term from AmiGO \cite{carbon2008amigo}.
Figure \ref{fig:GO_TERM_ROCS}
...
Indeed, queries can contain spurious nodes that are topologically distinct from one another - the result is that the $\mu_i(S)$ score for the query set will likely be very low.
...
For real world networks with small label sets, average AUC scores were above 0.88 for all the datasets tested in this study.
...
We are grateful for access to the University of Nottingham High Performance Computing Facility in support of this work.
This work was supported by the Biotechnology and Biological Sciences Research Council grant BB/L013940/1.
We would like to thank Nicole Pearcy and Jenna Reps for assistance with this manuscript.
