...
We also note that though each node can have multiple labels, in the tests performed in this study we only consider performance on individual labels.
...
We also note that we do not consider separate training and test sets in this study as the method does not use any examples when building the model from partitions of the graph.

Formally, the steps for this procedure with a given label set $S$ are outlined as follows:
\begin{itemize}
 \item Generate up to 120 unique subsets of size $s$, sampled without replacement (set $T$).
 ...
 \item Consider all nodes in nodes in $S$ but not in $S_t$ to be the \textit{tp} set.
 \item Consider each of the nodes in $V$ not in $S$ to be the \textit{tn} set.
 \item Generate an average ROC curve interpolated from all test subsets.
 \end{itemize}

Results presented are based on interpolated average ROC curves for all labels on each of the data sets. 
...
Consequently, we wish to to highlight that the method also serves as a useful network reduction tool where the quality of labels is not well defined.
