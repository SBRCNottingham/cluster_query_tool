...
@click.argument("seed")
@click.argument("results_folder")
def auc_compute_rwr(seed, n, mu, results_folder):
    pset = _base_params.copy()
    pset['n'] = int(n)
    pset['mu'] = float(mu)
    pset['seed'] = int(seed)
...
cd $WORK_DIR
python {_file} auc_compute_rwr {n} {mu:.2f} $JOB {results_folder}

"""

    workdir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    results_folder = os.path.join(workdir, 'hpc_results', "lfr_no_overlap_{}_rwr".format(n))
    ...
    for mu in np.linspace(0, 1, mu_steps):
        command_file_path = os.path.join(commands_path, "lfr_bm_{}_{:.2f}_rwr.sh".format(n, mu))
